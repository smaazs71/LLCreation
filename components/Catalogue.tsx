...
const Catalogue = ({searchParams}: any) => {
   ...
   const isDataEmpty = !Array.isArray(filteredProducts) || filteredProducts.length < 1 || !filteredProducts;
...
{filteredProducts?.map((product, i) => {return (
        (searchParams && searchParams.limit || 10) > i ? <ProductCard product={product} /> : ""
      )})}

    </div>
    <ShowMore
      pageNumber={(searchParams && searchParams.limit || 10) / 10}
      isNext={(searchParams && searchParams.limit || 10) < filteredProducts.length}
